	...
	<view class="page">
		...
		<view class="type-tabs">
			<view class="type-tab" :class="{ active: errorType === 'word' }" @click="errorType = 'word'">
				<text>单词错误</text><text class="count">{{ wordTotal }}</text>
			</view>
...
export default {
	...
	methods:{
		...
		modeCount(mode){return mode==='all'?this.wordTotal:Number(this.wordModeCounts[mode]||0)},
...
<style>
.page{min-height:100vh;background:#f7f5f0;padding:24rpx;box-sizing:border-box}.header{text-align:center;padding:22rpx 0 28rpx}.title{display:block;font-size:40rpx;font-weight:700;color:#1f3a5f}.subtitle{display:block;margin-top:10rpx;color:#8a8a8a;font-size:22rpx}.type-tabs{position:sticky;top:0;z-index:6;display:flex;background:#fff;padding:8rpx;border-radius:18rpx;margin-bottom:14rpx;box-shadow:0 4rpx 10rpx rgba(31,58,95,.06)}.type-tab{flex:1;display:flex;align-items:center;justify-content:center;gap:10rpx;padding:21rpx 10rpx;border-radius:13rpx;color:#555;font-size:29rpx}.type-tab.active{background:#1f3a5f;color:#fff}.count{min-width:34rpx;height:34rpx;line-height:34rpx;text-align:center;border-radius:18rpx;background:#eef2f7;color:#1f3a5f;font-size:21rpx}.type-tab.active .count{background:rgba(255,255,255,.2);color:#fff}.mode-scroll{position:sticky;top:100rpx;z-index:5;white-space:nowrap;background:#f7f5f0;margin-bottom:14rpx}.mode-tabs{display:inline-flex;gap:12rpx;padding:2rpx}.mode-tab{padding:15rpx 24rpx;border-radius:28rpx;background:#fff;color:#666;font-size:24rpx}.mode-tab.active{background:#0d9488;color:#fff}.card{background:#fff;border-radius:18rpx;padding:27rpx;margin-bottom:18rpx;box-shadow:0 4rpx 14rpx rgba(31,58,95,.07)}.word-line{display:flex;align-items:baseline;gap:12rpx}.word{font-size:36rpx;font-weight:700;color:#1f3a5f}.phonetic{color:#888;font-size:24rpx}.meaning{display:block;margin:10rpx 0 15rpx;color:#444;font-size:27rpx}.meta{display:flex;justify-content:space-between;align-items:center;color:#888;font-size:22rpx}.tag{padding:5rpx 13rpx;border-radius:9rpx;background:#fff0f0;color:#dc4c4c}.grammar-tag{background:#fff7e8;color:#b66b00}.actions{display:flex;justify-content:flex-end;margin-top:16rpx}.practice{margin:0;padding:0 28rpx;height:60rpx;line-height:60rpx;border-radius:12rpx;background:#1f3a5f;color:#fff;font-size:25rpx}.grammar-title{display:block;font-size:28rpx;font-weight:700;color:#1f3a5f}.sentence{display:block;margin:16rpx 0;color:#333;font-size:28rpx}.empty{text-align:center;padding:150rpx 0;color:#888}.empty-icon{display:block;font-size:72rpx;margin-bottom:18rpx}

.loading-list{padding-top:2rpx}.skeleton-card{margin-bottom:18rpx;padding:27rpx;border-radius:18rpx;background:#fff;overflow:hidden}.skeleton-line{height:24rpx;width:64%;margin:14rpx 0;border-radius:12rpx;background:linear-gradient(90deg,#edf0f2 25%,#f8f9fa 45%,#edf0f2 65%);background-size:300% 100%;animation:wrong-loading 1.2s ease-in-out infinite}.skeleton-line.wide{height:34rpx;width:42%}.skeleton-line.short{width:28%}@keyframes wrong-loading{0%{background-position:100% 0}100%{background-position:0 0}}
</style>