...
export default {
	...
	methods:{
		...
		async choose(option){if(this.answered)return;this.selected=option;if(option===this.correctAnswer)await this.completeCurrent(this.singlePractice?'回答正确，正在返回错题本':'回答正确，即将进入下一题');else{this.answered=true;this.passed=false;this.feedback='回答错误，请再练习一次';await markWordAsUnknown(this.current.word,this.mode).catch(()=>{})}},
		async checkWrite(){if(!this.current||this.answered)return;if(this.writeInput.trim().toLowerCase()===String(this.current.word).toLowerCase())await this.completeCurrent(this.singlePractice?'拼写正确，正在返回错题本':'拼写正确，即将进入下一题');else{this.answered=true;this.passed=false;this.feedback='拼写错误，正确答案是：'+this.current.word;await markWordAsUnknown(this.current.word,this.mode).catch(()=>{})}},
		retry(){this.selected='';this.answered=false;this.passed=false;this.feedback='';this.writeInput=''},
		async completeCurrent(message){if(!this.current)return;this.answered=true;this.passed=true;this.feedback=message;try{await markWordAsKnown(this.current.word,this.mode)}catch(error){this.passed=false;this.feedback='保存练习结果失败，请重试';return}setTimeout(async()=>{if(this.singlePractice)return uni.navigateBack();this.words.shift();if(!this.words.length)return this.finishQueue();await this.prepareQuestion()},650)},
		finishQueue(){uni.showToast({title:'该类型错题已完成',icon:'success',duration:900});setTimeout(()=>uni.navigateBack(),900)},
		playStandard(){if(!this.current?.word)return;const url='https://dict.youdao.com/dictvoice?audio='+encodeURIComponent(this.current.word)+'&type='+this.voiceType;uni.downloadFile({url,success:r=>{if(r.statusCode!==200||!r.tempFilePath)return uni.showToast({title:'语音加载失败',icon:'none'});if(this.audio){this.audio.stop();this.audio.destroy()}const audio=uni.createInnerAudioContext();this.audio=audio;audio.src=r.tempFilePath;audio.onCanplay(()=>audio.play());audio.onError(()=>uni.showToast({title:'语音播放失败',icon:'none'}))},fail:()=>uni.showToast({title:'语音加载失败',icon:'none'})})},
		...
		evaluateRecording(filePath){this.evaluating=true;uni.getFileSystemManager().readFile({filePath,encoding:'base64',success:async r=>{try{const evaluation=await evaluateSpeech(r.data,this.current.word,'read_word','mp3');const score=Number(evaluation?.score||0);if(score>=70)await this.completeCurrent('发音得分 '+score+(this.singlePractice?'，正在返回错题本':'，即将进入下一题'));else{this.answered=true;this.passed=false;this.feedback='发音得分 '+score+'，达到70分后完成';await markWordAsUnknown(this.current.word,this.mode).catch(()=>{})}}catch(error){this.feedback=error?.message||'评测失败，请重试'}finally{this.evaluating=false}},fail:()=>{this.evaluating=false;this.feedback='读取录音失败'}})}
	}
};
</script>
<style>
