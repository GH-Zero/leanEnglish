		...
		<view v-for="section in sections" :key="section.key" class="section">
			...
			<view class="badge-grid">
				<view v-for="badge in section.items" :key="badge.id || badge.name" class="badge-card" :class="{ unlocked: badge.unlocked, progressing: !badge.unlocked && badgeProgress(badge) > 0 }">
					<view class="badge-top"><view class="badge-icon">{{ badge.icon }}</view><text class="badge-state">{{ badge.unlocked ? '已解锁' : (badgeProgress(badge) > 0 ? '进行中' : '未开始') }}</text></view>
					<text class="badge-name">{{ badge.name }}</text><text class="badge-desc">{{ badge.description }}</text>
