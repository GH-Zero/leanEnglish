<template>
	<view class="container">
		...
		<view class="section adventure-section">
			<view class="section-heading"><view><text class="section-title">主线闯关</text><text class="section-subtitle">沿总地图依次完成核心学习模块</text></view><text class="plan-tag">{{ adventure.passedCount }}/{{ adventure.total }}</text></view>
			<view class="adventure-card" @click="goAdventure">
				<view class="adventure-icon">🗺️</view><view class="adventure-info"><text class="adventure-label">当前进度</text><text class="adventure-title">{{ adventure.current ? `第${adventure.current.level_no}站 · ${adventure.current.title}` : '全部成长路线已完成' }}</text><view class="adventure-track"><view :style="{width:adventurePercent+'%'}"></view></view></view><text class="adventure-go">进入地图 ›</text>
			</view>
		</view>
...
export default {
	data() {
		return {
			...
			reminderItems: [],
			adventure: { passedCount: 0, total: 10, stars: 0, current: null },
			stats: {
				streakDays: 0,
				totalWordsLearned: 0,
	...
	methods: {
		async loadAdventure() { try { this.adventure = await request('/adventure/levels'); } catch (_) {} },
