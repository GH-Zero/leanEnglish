<template>
  <view v-if="visible && current" class="achievement-overlay" @touchmove.stop.prevent>
    <view class="achievement-modal">
      ...
      <text class="achievement-kicker">成就解锁</text>
      ...
      <text v-if="current.reward" class="achievement-reward">🎁 奖励：{{ current.reward }}</text>
...
export default {
  data() { return { visible: false, current: null, queue: [], checking: false, unlockAudio: null }; },
  mounted() {
    uni.$on('achievement:check', this.scheduleCheck);
    this.scheduleCheck();
  },
  beforeUnmount() { uni.$off('achievement:check', this.scheduleCheck); if (this.timer) clearTimeout(this.timer); this.clearUnlockFanfare(); },
  methods: {
    ...
    showNext() { this.current = this.queue.shift() || null; this.visible = Boolean(this.current); if (this.visible && this.current) this.playUnlockFanfare(); },
    close() { this.visible = false; this.clearUnlockFanfare(); setTimeout(() => this.showNext(), 180); },playUnlockFanfare(){this.clearUnlockFanfare();try{const audio=uni.createInnerAudioContext();audio.autoplay=false;audio.volume=1;audio.src='/static/audio/fanfare-unlock.mp3';audio.onCanplay(()=>audio.play());audio.onEnded(()=>this.clearUnlockFanfare());audio.onError(()=>this.clearUnlockFanfare());this.unlockAudio=audio}catch(e){}},clearUnlockFanfare(){const audio=this.unlockAudio;this.unlockAudio=null;if(audio){try{audio.stop();audio.destroy()}catch(e){}}}
  }
};
</script>
<style scoped>
...
.achievement-reward{display:inline-block;margin-top:14rpx;padding:10rpx 20rpx;border-radius:22rpx;background:linear-gradient(135deg,#fff3d6,#ffe9b8);color:#a36b14;font-size:22rpx;font-weight:800}
</style>